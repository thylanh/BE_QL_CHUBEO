...
export class OrdersStoreService {
  ...
  async listMenuItems() {
    const result = await this.database.query<MenuRow>(
      `SELECT m.id, m.name, m.price::float8, m.category, m.description, m.image, m.created_at, m.active, mi.ingredient_id, mi.amount FROM menu_items m LEFT JOIN menu_item_ingredients mi ON mi.menu_item_id = m.id WHERE m.active = TRUE ORDER BY m.name`,
    );
  ...
  async createOrder(
    ...
    return this.database.transaction(async (client) => {
      const menuResult = await client.query<MenuRow>(
        `SELECT m.id, m.name, m.price::float8, m.category, m.description, m.image, m.created_at, m.active, mi.ingredient_id, mi.amount FROM menu_items m LEFT JOIN menu_item_ingredients mi ON mi.menu_item_id = m.id WHERE m.id = ANY($1::text[]) AND m.active = TRUE`,
